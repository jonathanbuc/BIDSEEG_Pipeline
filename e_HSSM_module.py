...
hssm_cfg       = inputs['Analysis']['hssm']
# HSSM settings
draws          = hssm_cfg['draws']
tune           = hssm_cfg['tune']
chains         = hssm_cfg['chains']
cores          = hssm_cfg['cores']
target_accept  = hssm_cfg['target_accept']
sampler        = hssm_cfg['sampler']
model_type     = hssm_cfg['model_type']
prior_settings = hssm_cfg['prior_settings']
link_settings  = hssm_cfg['link_settings']
# parameter formulas and fixeed parameters
formula_v      = hssm_cfg['formula_v']
formula_z      = hssm_cfg['formula_z']
t              = hssm_cfg['t']
...
def prep_hssm_data(df, cond_col, conditions):
    ...
    # accuracy coding 1 = correct, 0 = incorrect
    #df['response'] = df['corr'].map({1: 1, 0: -1}).astype(int)
...
def fit_hssm_hierarchical(df, condition_dict, formula_v, formula_z, model_type,
                           prior_settings, link_settings,
                           draws, tune, chains, cores, target_accept, sampler, t):
    """
    ...
    utils.log_msg(f'        model={model_type}, sampler={sampler}, formula_v="{formula_v}", formula_z="{formula_z}"')
...
def save_hssm_diagnostics(model, result_dir):
    ...
    import arviz as az
    axes = az.plot_trace(model._inference_obj)
...
if __name__ == '__main__':
    ...
    if not compute_hssm:
        ...
        model = fit_hssm_hierarchical(
            df_group, condition_dict, formula_v, formula_z, model_type,
            prior_settings, link_settings, draws, tune, chains, cores, target_accept, sampler, t
        )
        ...
        utils.log_update(log_df, 'hssm_formula_z', formula_z)
